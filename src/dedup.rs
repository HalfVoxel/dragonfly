...
//! `~/.dragonfly/dedup/` keyed by content hash and mirrored through a shared
//! GCS bucket (see [sync]), so each function hits the LLM once org-wide.
//! Confirmed false positives are recorded as pairwise exclusions (per origin
//! URL, shared across worktrees and, through the same bucket, across the
//! team) and never reported again.
...
// 0.90 finds only near-exact behavior matches; 0.85 still surfaces the
// "same family, maybe foldable" tier while cutting the noise 0.80 let
// through. The agent verifies every hint and dismissals persist.
...
/// Returns the summaries generated by this run (the shared-cache delta for
/// [sync::sync_up]).
async fn summarize_all(
    ...
    // Delta = the former misses now present in the cache. Computed from the
    // cache rather than accumulated per batch so the single-function retry
    // path in summarize_batch is covered too.
    let fresh: HashMap<String, String> = {
        let c = cache.lock().unwrap();
        missing
            .iter()
            .filter_map(|h| c.get(h).map(|s| (h.clone(), s.clone())))
            .collect()
    };
...
type EmbedMap = HashMap<String, Vec<f32>>;

/// Return an `embed_key -> unit vector` map covering every distinct key in
/// `fns`, fetching cache misses from Vertex, plus the vectors fetched by
/// this run (the shared-cache delta). `cache` is the already-loaded local
/// embedding cache; it is extended and persisted here.
async fn embed_all(
    fns: &[GoFn],
    mut cache: EmbedMap,
    quiet: bool,
) -> Result<(EmbedMap, EmbedMap), String> {
    ...
    if !missing.is_empty() {
        ...
        let fetched = Arc::try_unwrap(fetched)
            ...
            .unwrap_or_default();
        if !fetched.is_empty() {
            cache.extend(fetched.iter().map(|(k, v)| (k.clone(), v.clone())));
            save_embeddings(&cache);
        }
        if let Some(e) = first_err {
            return Err(e);
        }
        return Ok((cache, fetched));
    }
    Ok((cache, EmbedMap::new()))
}
...
async fn load_exclusions() -> Exclusions {
    load_exclusions_in(&repo_dir().await)
}

fn load_exclusions_in(dir: &Path) -> Exclusions {
...
async fn append_exclusions(new: &[ExclusionPair]) -> Result<(), String> {
    append_exclusions_in(&repo_dir().await, new)
}

fn append_exclusions_in(dir: &Path, new: &[ExclusionPair]) -> Result<(), String> {
    use std::io::Write as _;
    let path = dir.join("exclusions.jsonl");
    if let Some(dir) = path.parent() {
        std::fs::create_dir_all(dir).map_err(|e| e.to_string())?;
    }
...
async fn compute_report_inner(
    ...
    sync::sync_down(quiet).await;
    let new_sums = summarize_all(&mut fns, &kit, quiet).await?;
    let (vecs, new_vecs) = embed_all(&fns, load_embeddings(), quiet).await?;
    sync::sync_up(&new_sums, &new_vecs, quiet).await;
...
pub async fn cmd_dismiss(
    ...
    if !new_pairs.is_empty() {
        if let Err(e) = append_exclusions(&new_pairs).await {
            eprintln!("dedup dismiss: failed to record exclusions: {e}");
            return 1;
        }
        // Dismissals often happen with no dedup run afterward on this
        // machine, so push them to the shared cache right away.
        sync::sync_up(&HashMap::new(), &EmbedMap::new(), false).await;
    }
